...
if( jsonlab.jsonopt('ShowProgress',0,opt)==1)
...
function object = parse_object(varargin)
    ...
    parse_char('}');
    if(isstruct(object))
        object= jsonlab.struct2jdata(object);
    end
...
global pos inStr isoct
    ...
    object = cell(0, 1);
    dim2=[];
    arraydepth= jsonlab.jsonopt('JSONLAB_ArrayDepth_',1,varargin{:});
    pbar=-1;
    ...
    if next_char ~= ']'
	if( jsonlab.jsonopt('FastArrayParser',1,varargin{:})>=1 && arraydepth>= jsonlab.jsonopt('FastArrayParser',1,varargin{:}))
        ...
        try
         ...
         while 1
            newopt= jsonlab.varargin2struct(varargin{:},'JSONLAB_ArrayDepth_',arraydepth+1);
