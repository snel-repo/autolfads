...
%         objWorker
    ...
    methods
        function pbar = ProgressBar(N, message, varargin)
            ...
            [~, pbar.cols] = PBT_analysis.ProgressBar.getTerminalSize();
...
%             end
    ...
    methods(Static)
        function demo(N, varargin)
            ...
            pbar = PBT_analysis.ProgressBar(N, varargin{:});
            for i = 1:N
                pbar.update(i);
...
%                 end
        ...
        function demoNested()
            ...
            pi = PBT_analysis.ProgressBar(ni, 'Outer loop');
            for i = 1:ni
                pi.update(i);
                pj = PBT_analysis.ProgressBar(nj, 'Inner loop');
                for j = 1:nj
                    pj.update(j);
                    pause(0.001);
                end
